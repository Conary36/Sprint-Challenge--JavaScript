...
for(let i = 0; i < graduates.length; i++){
  ...
  newUniversities = universities.sort();
}
}
listAlpha(graduates);
...
let contactInfo = [];
  contactInfo = graduates.forEach(graduates => {
...
let unisWithUni = [];
unisWithUni = graduates.map((graduates) =>{
  return {
    'Uni': graduates.university
  };
});
...
*/
let displayNames = [];
  displayNames = zooAnimals.forEach(zooAnimals => {
...
lowCaseAnimalNames = zooAnimals.map((zooAnimals) =>{
  return{
    'animal': zooAnimals.animal_name.toLocaleLowerCase()
  };
})
...
let lowPopulationAnimals = [];
  lowPopulationAnimals = zooAnimals.filter((lowNum) => {
    return lowNum.population <= 5;
  })
